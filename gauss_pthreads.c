 ...
 */
 ...
 //this function is called by each thread to compute for their specified portion of rows
 void* GaussianParallel(void* p){
 
 int n = *(int *)p; // setting n to the thread number
 
 int norm, row, col;  
 float multiplier;

 
 /* setting the starting row and and ending row for each thread*/
 int starting = normglobal + (n * divider) + 1;
 int ending = starting + divider;
 ending = ( ending >= N || n == numThreads -1) ? N : ending ; // setting ending to N if it exceeds N or its the last thread
 
 
 norm = normglobal;                      // retrieving the current norm value from the global 
 
    for (row = starting; row < ending; row++) 
    {
